...
class FeedPicMapperTest {
    ...
    @Test
    void insFeedPicLongPicStringLengthThrowException() {
        FeedPicDto givenParam = new FeedPicDto();
        givenParam.setFeedId(1L);
        givenParam.setPics(new ArrayList<>(1));
    ...
    void insFeedPic(){
        ...
        int actualAffectRows = feedPicMapper.insFeedPic(givenParam);
        ...
        assertAll(
                ...
                , () -> assertEquals(givenParam.getPics().size(), actualAffectRows)
                ...
                , () -> assertTrue(feedPicListAfter.stream() //스트림 생성 Stream<FeedPicVo>
                                            .map(FeedPicVo::getPic) //똑같은 크기의 새로운 변환 Stream<String> ["a.jpg", "b.jpg", "c.jpg"]
                                            .filter(pic -> picList.contains(pic)) //필터는 연산의 결과가 true인 것만 뽑아내서 새로운 스트림 변환 Stream<String> ["a.jpg", "b.jpg", "c.jpg"]
                                            .limit(picList.size()) //스티림 크기를 제한, 이전 스트림의 크기가 10개인데 limit(2) 2개짜리 스트림이 변환된다.
                                            .count() == picList.size())
